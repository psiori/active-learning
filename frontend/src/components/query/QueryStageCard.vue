  ...
  <section class="stage-card">
    <div class="stage-header">
      ...
      <button
        ...
        :title="isDirty ? 'Save query settings to config file' : 'No unsaved changes'"
        @click="handleSave"
      >
...
import QueryStageForm from './QueryStageForm.vue'
...
defineProps({
  ...
  cancellingQuery: { type: Boolean, default: false },
...
async function handleSave() {
  ...
  try {
    await store.saveQueryConfig()
    saveState.value = 'success'
  } catch (err) {
    console.error('Failed to save query config:', err)
